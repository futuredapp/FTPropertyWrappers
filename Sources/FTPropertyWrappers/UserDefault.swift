...
public final class DefaultsStore<Value: Codable> {
    ...
    /// Key for encoled property, programmer is responsible for keeping this key unique in target UserDefaults
    /// domain.
    ...
    /// - Parameters:
    ///   - key: Key for encoled property, programmer is responsible for keeping this key unique in target
    ///   UserDefaults domain
    ///   - defaultValue: User defaults store used for this property
    ///   - defaults: Default valure returned by enclodes property getter in case, that property is not found in
    ///   the UserDefaults
